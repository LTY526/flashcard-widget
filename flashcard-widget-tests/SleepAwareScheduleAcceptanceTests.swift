...
struct SleepAwareScheduleAcceptanceTests {
    ...
    @Test("Schedule captures persisted current and future rows without mutation")
    func schedulePartitionAndPagination() throws {
        ...
        let snapshot = try ScheduleSnapshot.load(deckID: deck.persistentModelID, from: context.container)
        #expect(snapshot.isValid)
        #expect(snapshot.upcoming.first?.sequence == deck.highestReachedSequence)
        #expect(snapshot.upcoming.first?.isCurrent == true)
        #expect(snapshot.past.map(\.sequence) == Array((1...12).reversed()))
        #expect(snapshot.scheduleHorizon == snapshot.upcoming.last?.projectedAt)
        #expect(scheduleSnapshot(deck) == before)
    }

    @Test("Schedule refresh captures a newly saved graph")
    func schedulePartitionRefresh() throws {
        let (context, deck) = try fixture(interval: 30)
        let start = date(year: 2026, month: 1, day: 1, hour: 10)
        try DeckScheduler.ensureSchedule(for: deck, in: context, now: start)
        try context.save()
        let old = try ScheduleSnapshot.load(deckID: deck.persistentModelID, from: context.container)
        try DeckScheduler.next(deck, in: context, now: start.addingTimeInterval(60))
        try context.save()
        let refreshed = try ScheduleSnapshot.load(deckID: deck.persistentModelID, from: context.container)
